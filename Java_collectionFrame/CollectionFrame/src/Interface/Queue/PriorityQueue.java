...
/*
 * 우선순위 큐는 각 요소들이 각각의 우선 순위를 갖고있고, 요소들의 대기열에서 
 * 우선순위가 높은 요소가 우선순위가 낮은 요소보다 먼저 제공되는 자료구조이다.
 * 
 * 우선순위 큐 = 힙이라고 생각할 수도 있지만, 개념자체는 다르다.
 * 
 * 힙이라는 방법으로 우선순위 큐를 구현하는 것이다.
 * 
 * 즉,
 * 힙에서의 최대 혹은 최소값을 빠르게 찾는 방법을 통해
 * 우선순위 큐를 구현하는 것.
 * 
 * 그렇기 때문에 힙의 코드와 우선순위 큐의 코드가 거의 비슷할 것이다.
 * 
 */

/*
 * 우선순위 큐는 배열 기반으로 구현된 힙으로 구현한다.
 * 
 */
public class PriorityQueue<E> implements Queue_basic<E> {
	...
	/*
	 * offer 메서드 구현
	 * 힙에서는 add 메서드로 구현하였지만, queue 인터페이스를 상속받았으므로
	 * offer 메서드로 구현하였다.
	 * 
	 * 힙과 마찬가지로, Comparator과 Comparable 중 어떤 것으로 정렬하는지에 따라
	 * 나눈다.
	 */
	...
	/*
	 * 마찬가지로 데이터를 삭제할때는 remove 메서드 대신
	 * poll 메서드를 구현한다.
	 */
	@Override
	public E poll() {
